...
function cargarCheckboxes(ids) {
    ...
    let table="<tr><th>Imagen</th><th>Nombre</th><th>Picante</th><th>Seco</th><th>Dulce</th><th>Amargo</th><th>Ácido</th></tr>";
        ...
        for (let y = 0; y < ids.length; y++){
            if (x[i].getElementsByTagName("id")[0].textContent == ids[y]){
                ...
                if(x[i].getElementsByTagName("ácido").length > 0){
                    table += x[i].getElementsByTagName("ácido")[0].textContent + "</td></tr>";
                } else {
                    table += x[i].getElementsByTagName("ácido4ta")[0].textContent + "</td></tr>";
                }
            }
        }
    }
